...
import ContentPreparationBanner from './ContentPreparationBanner'
...
const STUDY_MODES = {
  flashcards: { icon: BookOpen, label: 'Flashcards', color: 'from-indigo-500 to-violet-500' },
  chat: { icon: MessageSquare, label: 'Chat', color: 'from-blue-500 to-cyan-500' },
  mindmap: { icon: Network, label: 'Mind Map', color: 'from-emerald-500 to-teal-500' },
  podcast: { icon: Mic, label: 'Podcast', color: 'from-violet-500 to-purple-500' },
}

export default function StudyGuideTab({ plans, onNavigateToMode }: StudyGuideTabProps) {
    ...
    return (
      <div className="text-center py-20">
        ...
        <p className="text-white/50 max-w-md mx-auto">
          Create a study plan first to see your personalized study guide with daily content.
        </p>
      </div>
    )
  }
    ...
    <div className="space-y-6">
      ...
      {!loading && !error && guideData && (
        <>
          ...
          {/* Prepare Today's Content Button */}
            ...
            {guideData.weeks.map((week) => (
              <StudyGuideWeek
                ...
                onRefreshDay={async (date) => {
                  // Trigger content refresh for a specific day
